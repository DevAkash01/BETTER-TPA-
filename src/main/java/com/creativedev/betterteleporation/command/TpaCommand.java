...
public final class TpaCommand implements CommandExecutor {
    ...
    public TpaCommand(BetterTeleporation plugin, ConfigManager configManager,
                      TeleportRequestManager requestManager, TeleportManager teleportManager,
                      CooldownManager cooldownManager, CombatManager combatManager,
                      DialogManager dialogManager, AutoAcceptManager autoAcceptManager,
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        if (args.length > 0) {
            ...
            if (firstArg.equals("reload")) {
                ...
                plugin.reloadPlugin();
                MessageUtil.sendMessage(sender, "admin.reload-success");
                return true;
            }
        }
        ...
        if (cooldownManager.isOnCooldown(player, "tpa")) {
            long remaining = cooldownManager.getRemainingSeconds(player.getUniqueId(), "tpa");
        ...
        if (!settingsManager.isTpaAllowed(target.getUniqueId())) {
            MessageUtil.sendMessage(player, "error.target-tpa-disabled", Placeholder.parsed("player", target.getName()));
            return true;
        }

        if (!isConfirmed && configManager.isDialogEnabled()) {
            dialogManager.openTpaConfirmation(player, target);
            return true;
        }
    ...
    private void sendRequest(Player player, Player target) {
        RequestResult result = requestManager.createRequest(player, target, TeleportRequestType.TPA);
        switch (result) {
            case SUCCESS -> {
                cooldownManager.setCooldown(player.getUniqueId(), "tpa", configManager.getCooldownTpa());
                ...
                MessageUtil.sendMessage(player, "request.sent", Placeholder.parsed("player", target.getName()));
